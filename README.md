...
### 2026-07-06 — Harness 制度層完備：模型調度守則與判斷力外化（Fable 5 一次性 session）

> 以一次性 Fable 5 配額執行制度建設而非日常任務：將高階模型的調度策略與判斷準則
> 固化為 `.knowledge/harness/` 制度文件套件，使後續各級模型（Opus / Sonnet / Haiku）
> 的每個 session 皆可直接沿用。

- **快速診斷**（`diagnosis.md`）— 系統性量測 token 洩漏、注意力發散、錯誤來源各前三名，附實測數據與對應修法路由；為套件內其餘文件的問題定義來源
- **模型調度守則**（`model-dispatch.md`）— 確立「指揮官不下場」原則（大量讀取、掃 repo、批次改檔一律委派 subagent，主對話僅收結論與 `檔案:行號`）；派工三件套（目標動機／驗收條件／回報格式）；升降級熔斷路徑：haiku 單次失敗即升級、sonnet 同一子任務連續兩次失敗攜完整失敗軌跡升 opus、全案至多兩次升級；已解模式降回廉價模型批次套用；驗收一律由 fresh-context agent 執行（驗證不自驗）
- **判斷力外化**（`judgment-rubrics.md`）— 五項判準：模型升級時機、完成認定、向使用者升級提問、換路訊號（區辨「該重試」與「方向錯誤」）、品質底線驗證。每項判準均含可觀察觸發訊號、檢查表、正例與反例
- **委派模板**（`delegation-templates.md`）— 搜尋／實作／重構／研究／審查五類任務的標準化派工 prompt，內建驗收條件填空（未填不得派工）、環境提示段與回報合約；附派工後抽查檢查表
- **維護協議**（`maintenance-protocol.md`）— 制度檔案三級權限分級（綠區可自改／黃區須提案核可／紅區不得變更）、修改流程、教訓回寫路徑、膨脹精簡門檻——防止弱模型憑印象劣化制度本體
- **給未來 session 的信**（`letter-to-future-sessions.md`）— 三項未被詢問但關鍵的環境事實（警報存活性優先於規則數量、本環境無長期記憶、老闆的稀缺資源為注意力）；五種制度退化模式與預防；harness 能力邊界之誠實聲明（品味與模糊判斷不可制度化，僅可保證「不擅自定稿」）
- **CLAUDE.md 雙層改寫** — 專案級由 84 行精簡至 56 行，重構為「按情境路由」索引；工作區根層新增「模型調度與委派」節並列入跨專案鐵律第 5 條。原檔皆備份於 `.knowledge/harness/backups/`
- **決策表銜接** — `decision-tables.md` 表 7 補齊與維護協議的雙向引用，消除審批流程繞過路徑
- **對抗審查** — 全部產出交由 fresh-context Sonnet agent 獨立審查（刻意以實際執行等級的模型模擬讀者），檢出 3 項規則衝突、2 項數據失實、4 處歧義語句、3 項模板缺陷，全數修復；審查報告全文入庫（`review-findings-2026-07-06.md`）
- **驗證** — 全數檔案 read-back 核對；文件引用之路徑、指令、skill 名逐一以 `test -f` 實證（零缺陷）；merge commit `31fb0d9` 已推送 main
...
- **重要的保護只靠「口頭規定」** — 電腦上有兩份一樣的專案：一份給 AI 工作用、一份是老闆自己驗收用的乾淨版。規則說 AI 不准碰驗收版，但這只是寫在說明文件裡的一句話，AI 忘了就沒了。
